...
- [x] 7.5.1 v0.2 dogfood (deploy-input build #19) revealed `/input/<id>/submit` is rejected by modern Jenkins ("Rejected by admin"). Wfapi `pendingInputActions[].proceedUrl` advertises `/wfapi/inputSubmit?inputId=<id>` as the correct path. Plumbed `proceedUrl` through `pendingInputItem` → `SubmitInput(…, proceedURL, …)`, prefer it when non-empty, fall back to `/input/<id>/submit` for older servers. Live-verified build #21 → SUCCESS, `decision: ENV=prod DRY_RUN=false`. Tests added: `Test_Client_SubmitInput_PrefersProceedURL`, `Test_BuildInput_UsesWfapiProceedURL`. Docs updated: `docs/schema.md §3.9` endpoint matrix.
- [x] 7.6 Tag and release v0.2.0 once dogfood validates the new flag against ≥1 real-world parameterized input pipeline.
